...
linear_regression <- function(formula, basevar, data) {
  ...
  pm <- emmeans(m, "cond",
    at = list(pradas_t0 = mean(used[[basevar]], na.rm = TRUE),
    strata = names(which.max(prop.table(table(used$strata))))))
  pm <- as.data.table(pm)
  ...
  c("N_total" = n_total,
    ...
    "Std_CI_low" = std_lcl,
    "Std_CI_high" = std_ucl,
    "Ctrl_Post_Mean" = pm[cond == 0, emmean],
    "Ctrl_Post_CI_low" = pm[cond == 0, lower.CL],
    "Ctrl_Post_CI_high" = pm[cond == 0, upper.CL],
    "Tx_Post_Mean" = pm[cond == 1, emmean],
    "Tx_Post_CI_low" = pm[cond == 1, lower.CL],
    "Tx_Post_CI_high" = pm[cond == 1, upper.CL])
}
